...
export const viewModelTestDefinitions = {
  'viewModel() basic functionality': (file: RiveFile) => {
  ...
  'replaceViewModel() replaces and shares state': (file: RiveFile) => {
    const vm = file.defaultArtboardViewModel();
    const instance = vm?.createDefaultInstance();
    expect(instance).toBeDefined();
...
export const viewModelTests: TestSuite = {
  ...
  getTests: (file: RiveFile) => {
    return Object.entries(viewModelTestDefinitions).map(([name, testFn]) => ({
      name,
      run: async () => {
        testFn(file);
