...
// Anchor inside expo57-example so imports from shared example/src files
// resolve dependencies from expo57-example/node_modules instead of
// example/node_modules (which is blocked by withBlockedSiblingDeps).
const expo57Anchor = path.join(__dirname, 'node_modules', '_module_anchor');
...
const configWithAlias = {
  ...
  resolver: {
    ...
    resolveRequest: (context, moduleName, platform) => {
      ...
      // expo57-example's node_modules so blocked sibling deps don't
...
const expo55ExampleDir = path.resolve(root, 'expo55-example');
