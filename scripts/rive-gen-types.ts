 ...
 *   import gameRiv from './assets/game.riv';                       // typed as RiveFileSource<GameSchema>
...
// A plain object type, not an intersection with RiveFileSchema: the base's
// index signatures would make every artboard, view model and enum name valid.
...
/**
 * A map keyed by names from the .riv file. Null-prototype, so a name like
 * `__proto__` becomes an own key instead of replacing the prototype.
 */
export function nameMap<V>(): Record<string, V> {
